...
func UploadFile(c *gin.Context) {
	...
	//}
	//存储到后端的OSS里面
	utils.OSSUploadHandler(c)

	//更新数据库相关信息
	//url := utils.MakeFileURL(file.Filename)
	url := utils.OSSMakeFileURL(file.Filename)

	//插入一条信息 到 文件信息表
	err := opdb.InsertRecordToFileInfo(md5_, file.Filename, url, fileExt, size, 1)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
			"code": 1,
		})
...
func DealFile(c *gin.Context) {
	...
	if cmd == "share" {
		...
		if count == 0 {
			...
			//OSS删除文件
			utils.OSSDelFile(storagefileld)
		}
		c.JSON(http.StatusOK, gin.H{
			"code": 0,
		})
